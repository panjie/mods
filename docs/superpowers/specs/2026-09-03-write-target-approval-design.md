...
审批矩阵之前增加独立的命令可审查性约束。静态证明只读的调用仍免审；
不透明或复合命令不能仅靠 LLM 的 read 判断越过结构约束。最多纠正两次，
仍不合格则停止，Minimal 不跳过此约束，显式 never 仍保留免审语义。
不存在通用的脚本执行通道：不透明或解释器包裹的内容必须拆分为分立的
单命令调用，不得藏进解释器参数、临时文件或编码参数。详见命令可审查性设计。
...
| 未知 effect，目标全部在安全临时目录 | 允许 | 允许 | 允许 |
| 未知 effect，含普通/动态/远端目标 | 每次询问，仅可 Allow once | 每次询问，仅可 Allow once | 允许 |

空写目标、无法解析的远端都 fail closed。未知 effect 映射为写，但位置规则同样适用：已知目标全部位于安全临时目录时直接允许；其余目标首次仍需评审。若目录和 origin 已明确且没有未解析目标，auto 模式允许按这些目标选择 Always allow，并复用会话规则。一次调用同时写本地和
