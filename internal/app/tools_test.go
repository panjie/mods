...
func TestBuildToolRegistryPowerShellRun(t *testing.T) {
	...
	cfg.WebSearch = false
	registry, err := buildToolRegistry(context.Background(), &cfg, websearch.Config{}, "hello", nil, nil)
	if err != nil {
...
func TestToolCapabilities(t *testing.T) {
	...
	cfg.BuiltinTools.SequentialThinking = true
	cfg.WebSearch = true
	registry, err := buildToolRegistry(context.Background(), &cfg, websearch.Config{Provider: "duckduckgo"}, "hello", nil, nil)
	if err != nil {
		t.Fatalf("build registry: %v", err)
	}
