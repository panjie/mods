...
- **复合**（→ 整体 fail-closed 保持动态）：尾部结束后紧跟 `$`/转义符（相邻展开拼接）；引用后紧跟引号且引号后还有路径字符（`$FOO/x"suf"` 拼接）；braced 引用后紧跟名字字符（`${FOO}suffix` 拼接）。**闭合引号后是空白/分隔符视为安全终止**（`"$FOO/x" -Tail 5` 是最常见的合法形态）。**PS 方言中闭合引号后的 `(` / `)` 同样视为安全终止**（`)` 结束分组表达式、`(` 开启新参数；括号仅在变量名内部如 `${env:ProgramFiles(x86)}` 才是名字字符）。
