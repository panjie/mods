...
### 2026-06-25：自我进化开发分支命名 eva

- 决定：自我意识与自我进化相关的开发工作在 `eva` 分支上推进（原 `夏娃` 分支已重命名）。
- 理由：分支名使用 ASCII、简短、可输入，避免中文分支名在 CI、脚本和 URL 中的兼容性问题，同时名字与“Eve/自我进化”概念保持呼应。
- 范围：仅分支名变更，代码、模块路径（`github.com/panjie/mods`）和 evaluation 状态不变。
