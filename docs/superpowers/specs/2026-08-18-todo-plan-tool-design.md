...
## 8 右侧常驻清单（2026-09-09 更新，取代内联展示）

- `todo_write` 成功后仅替换 `Mods.todoItems`，不再向正文或 display blocks 追加清单；原有会话保存与恢复机制不变。
- TTY 宽度至少 100 列、高度至少 10 行时，清单固定在右侧，占终端宽度的三分之一，最多 40 列。左侧正文独立滚动，状态栏、审批与输入面板仍占底部全宽。
- 面板中的步骤使用单行省略；高度不足时围绕当前步骤显示，并标注上下方隐藏步骤数。完成后保留至下一轮开始。
- 小窗口使用底部单行进度，不重复输出清单；调整窗口尺寸时自动切换布局。
- raw、minimal、hide-tool-status 不显示清单；非 TTY 继续输出原有 stderr 摘要。


## 9 底部常驻清单（2026-09-09 调整，取代右侧侧栏）

- 清单移至底部状态栏上方，正文恢复终端全宽，并在清单上方独立滚动。
- TTY 至少 40 列、10 行时显示清单面板；面板最多占终端高度的三分之一且不超过 8 行，长清单仍围绕当前步骤显示。
- 审批或输入面板保持底部全宽，优先获得所需空间；剩余空间不足时清单压缩为单行进度，必要时暂时隐藏。
- 小窗口继续使用紧凑进度行；原位更新、输出隔离、模式门控和会话恢复行为保持不变。


## 10 避免 todo 出现时撑满终端（2026-09-09 修正）

- 正文视口高度取实际内容高度与可用高度的较小值，不再为底部清单补齐整屏空行。
- 无正文时只显示清单和状态栏；短正文下方直接显示清单，保留终端上方已有内容。
- 仅正文超出可用高度时启用滚动；清单继续在状态栏上方原位更新。
