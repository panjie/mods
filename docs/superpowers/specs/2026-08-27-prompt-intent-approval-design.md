...
审批子系统（`2026-07-02-unified-directory-approval-design`）的核心原则是：**授权只来自可验证的工具调用本地文件系统事实，永远不来自语义声明**。最初当用户说"提交并推送本地修改"时，`git add -A` 仍会被拦下——它是 write 效应且没有具体目标目录，按矩阵 fail-closed 询问（"Modify an unknown target"）。现在直接进程中可确定仓库位置的常见 Git 工作树/index 写操作走静态快路径；本设计的写范围分类器保留为其他空目录写命令的后备。
...
| 空目录写 | 先使用静态命令范围；仍为空时由写范围分类器判定 workspace/external/unknown/无本地写，fail-closed |
