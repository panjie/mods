...
- Config defaults are generated from embedded `internal/config/config_template.yml`; when adding settings, update the config structs, `Help` map, template, config tests, and `internal/prompts/identity.md` together.
...
- Self-help identity prompt: `internal/prompts/identity.md` is the comprehensive LLM knowledge base for mods usage. When adding a CLI flag that goes into the `Help` map and `initFlags`, copy its description into identity.md under the matching category (`flagCategory` constants in `internal/cli/flags.go`). When adding a top-level config key, document it in identity.md's config file structure section. Tests `TestIdentityCoversAllFlags` and `TestIdentityCoversConfigKeys` in `internal/prompts/prompts_test.go` catch omissions.
