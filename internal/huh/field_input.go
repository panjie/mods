...
// Cursor returns the real terminal cursor relative to the rendered field.
func (i *Input) Cursor() *tea.Cursor {
	...
	if i.title.val != "" || i.title.fn != nil {
		prefix.WriteString(styles.Title.Render(wrap(i.title.val, maxWidth)))
		if !i.inline {
			prefix.WriteString("\n")
		}
	}
	if i.description.val != "" || i.description.fn != nil {
		prefix.WriteString(styles.Description.Render(wrap(i.description.val, maxWidth)))
		if !i.inline {
			prefix.WriteString("\n")
		}
	}
	return cursorInStyle(cursorAfterPrefix(i.textinput.Cursor(), prefix.String()), styles.Base)
}
